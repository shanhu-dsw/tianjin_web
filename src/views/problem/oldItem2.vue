...
export default {
  ...
  methods: {
    ...
    getRandomObjects(arr) {
      ...
      // 取前9个对象
      var randomObjects = flatArr.slice(0, 9);
    ...
    getmessage_user(problem_uuid) {
      let query = {
        rows: 1000,
        offset: 0,
        manage_department_uuid_like: this.unitMultipleValue.uuid,
      };
      getAdminShort(query).then((response) => {
        ...
        const message_data = {
          from_uuid: this.admin.uuid,
          to_uuid: "",
          title: "发现" + this.problemType.name + "的问题",
          content: msg,
          remark: problem_uuid,
        };
